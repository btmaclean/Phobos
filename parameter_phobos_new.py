...
plotornot = 0 # Switch to 1 to see all MOOG plots
...
def xi_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,RW_slope_err,xirange):
    ...
    RWslopem = psumlist[1]
    print RWslopeph, RWslopep, RWslopem
    if RWslopem == 999:
        xi = round(2.22-0.322*logg, 2)
        return xi, 999
...
def recalc_ph_logg(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod):
    agb_mass = 0.60
    rgb_mass = 0.76
    print('***WARNING YOU ARE USING ' + str(agb_mass) + ' and ' + str(rgb_mass))

    psumlist = psum(name,Teff,logg,xi)
    fe_h = psumlist[3] - 7.5
    if 'AGB' in name:
        if Teff <= 4677:
            logg = round(4.44+math.log10(agb_mass)+0.4*(Vmag-dist_mod+((-5.531*10**(-2))/(math.log10(Teff)-3.52))-0.6177+(4.42*(math.log10(Teff)-3.52))-2.669*((math.log10(Teff)-3.52)**2)+(0.6943*(math.log10(Teff)-3.52)*-1.1)-0.1071*-1.1-(8.612*10**(-3))*(-1.1**2)-4.72)+4*math.log10(Teff)-15.0447, 2)
        else:
            logg = round(4.44+math.log10(agb_mass)+0.4*(Vmag-dist_mod+((-9.93*10**(-2))/(math.log10(Teff)-3.52))+2.887*10**(-2)+(2.275*(math.log10(Teff)-3.52))-4.425*((math.log10(Teff)-3.52)**2)+(0.3505*(math.log10(Teff)-3.52)*-1.1)-(5.558*10**(-2))*-1.1-(5.375*10**(-3))*(-1.1**2)-4.72)+4*math.log10(Teff)-15.0447, 2)
    else:
            if Teff <= 4677:
                logg = round(4.44+math.log10(rgb_mass)+0.4*(Vmag-dist_mod+((-5.531*10**(-2))/(math.log10(Teff)-3.52))-0.6177+(4.42*(math.log10(Teff)-3.52))-2.669*((math.log10(Teff)-3.52)**2)+(0.6943*(math.log10(Teff)-3.52)*-1.1)-0.1071*-1.1-(8.612*10**(-3))*(-1.1**2)-4.72)+4*math.log10(Teff)-15.0447, 2)
            else:
                logg = round(4.44+math.log10(rgb_mass)+0.4*(Vmag-dist_mod+((-9.93*10**(-2))/(math.log10(Teff)-3.52))+2.887*10**(-2)+(2.275*(math.log10(Teff)-3.52))-4.425*((math.log10(Teff)-3.52)**2)+(0.3505*(math.log10(Teff)-3.52)*-1.1)-(5.558*10**(-2))*-1.1-(5.375*10**(-3))*(-1.1**2)-4.72)+4*math.log10(Teff)-15.0447, 2)
    if logg < 0.0:
        logg = 0.0
    return logg, fe_h
...
def param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange):
    ...
    moog(star,name,feelements,location,plotornot)
    psumlist = psum(name,Teff,logg,xi)
    iteration = [(Teff,Teffuncert,logg,xi,xiuncert,psumlist[3],psumlist[4],psumlist[6],psumlist[7],psumlist[0],psumlist[1])]
    with open('phobos_iterations/{}.iterations'.format(name),'a') as f_handle:
        np.savetxt(f_handle,iteration,delimiter=' ',fmt='%s')
    return (Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1])

Trange = 200
xirange = 0.2
Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)
Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)
Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)
...
if abs(psumlist[1]) >= 0.003:
    Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)
if abs(psumlist[0]) >= 0.003:
    Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)
if abs(psumlist[1]) >= 0.002:
    Teff, Teffuncert, logg, xi, xiuncert, fe_h, psumlist[0], psumlist[1] = param_analysis(name,location,Teff,logg,xi,Vmag,fe_h,star,feelements,dist_mod,Trange,xirange)

if abs(psumlist[0]) >= 0.01:
    print psumlist[0]
    sys.exit('\nPhobos failed due to Teff. Check moog_input file for potentially saturated lines\n')
if abs(psumlist[1]) == 999:
    print 'Microturbulence photometrically determined'
elif abs(psumlist[1]) >= 0.02:
    print psumlist[1]
    plotornot = 1
    model(name,location,Teff,logg,xi,fe_h)
    moog(star,name,feelements,location,plotornot)
    X_lines_summary(name,location,feelements)
    sys.exit('\nPhobos failed due to v_t. Check moog_input file for potentially saturated lines\n')
...
psumlist = psum(name,Teff,logg,xi)
fe_h = psumlist[3] - 7.5
print '\nSpec Params: Teff\tTeff_u\tlogg\txi\txi_u\tFeI\tFeI_u\tFeII\tFeII_u'
print 'Spec Params: {}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(Teff,Teffuncert,logg,xi,xiuncert,psumlist[3],psumlist[4],psumlist[6],psumlist[7])
